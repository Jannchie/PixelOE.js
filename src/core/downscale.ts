...
function kMeansCluster(pixels: Array<[number, number, number]>, k: number = 2, maxIterations: number = 10): Array<[number, number, number]> {
  ...
  let minR = pixels[0][0]; let maxR = pixels[0][0]
  let minG = pixels[0][1]; let maxG = pixels[0][1]
  let minB = pixels[0][2]; let maxB = pixels[0][2]
  ...
  for (let iter = 0; iter < maxIterations; iter++) {
    ...
    const clusters: Array<Array<[number, number, number]>> = new Array(k).fill(null).map(() => [])
