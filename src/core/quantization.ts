...
export function findNearestCentroid(
  ...
  for (let i = 0; i < centroids.length; i++) {
    ...
    for (let c = 0; c < pixel.length; c++) {
      const diff = pixel[c] - centroids[i][c];
      distance += diff * diff;
    }
...
export function kMeansIteration(
  ...
  const sums = Array(numCentroids).fill(null).map(() => new Array(numChannels).fill(0));
  ...
  for (let i = 0; i < pixels.length; i++) {
    ...
    for (let c = 0; c < numChannels; c++) {
      sums[assignment][c] += pixels[i][c] * weight;
    }
  }
