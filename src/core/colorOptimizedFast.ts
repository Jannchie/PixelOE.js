...
function fastSampledColorMatch(source: PixelImageData, target: PixelImageData, level: number): PixelImageData {
  ...
  const sampleSize = Math.min(10000, Math.floor(source.width * source.height / 100))
  ...
  for (let y = 0; y < source.height; y++) {
    for (let x = 0; x < source.width; x++) {
      ...
      if (sourceStd[0] > 0) newL = ((cvL - sourceMean[0]) / sourceStd[0]) * targetStd[0] + targetMean[0]
      if (sourceStd[1] > 0) newA = ((cvA - sourceMean[1]) / sourceStd[1]) * targetStd[1] + targetMean[1]
      if (sourceStd[2] > 0) newB = ((cvB - sourceMean[2]) / sourceStd[2]) * targetStd[2] + targetMean[2]
